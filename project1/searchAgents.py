...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if state[0] in self.corners:
            ...
            return len(state[1]) == 4 #The number of visited corners equals to 4
        return False
        util.raiseNotDefined()
        """
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        distance = util.manhattanDistance(current, food)
        distances.append(distance)

    index = distances.index(min(distances))
    current = foodList[index]
    food_distances = []
    ...
    for food in foodList: 
        ...
        min_x = min(current[0], food[0])
        max_x = max(current[0], food[0])
        min_y = min(current[1], food[1])
        max_y = max(current[1], food[1])


        for x in range(min_x, max_x):
            if walls[x][food[1]]: 
                wall1_cost += 2
            if  walls[x][current[1]]:
                wall2_cost += 2
        for y in range(min_y, max_y):
            if walls[food[0]][y]:
                wall2_cost += 2
            if walls[current[0]][y]:
                wall1_cost += 2

        distance += min(wall1_cost, wall2_cost)
        food_distances.append(distance)


    cost = min(distances) + max(food_distances)
    return cost
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        return search.breadthFirstSearch(problem) #breadthFirstSearch starts from the closest dot 
        util.raiseNotDefined()
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        return self.food[x][y] #return true if food on the corresponding coordinate

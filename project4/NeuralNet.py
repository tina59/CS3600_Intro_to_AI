...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        inActs.insert(0, 1.0)
        sum = self.getWeightedSum(inActs)
        del inActs[0]
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
            tmp = weight
            weight += (alpha * inActs[index] * delta)
            weights.append(weight)
            totalModification += abs(weight - tmp)
            index += 1
        del inActs[0]
        self.weights = weights
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            perceptors = []
            for perceptor in layer:
                perceptors.append(perceptor.sigmoidActivation(curr_input))
            curr_input = perceptors
            key.append(perceptors)
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    while (not weightMod or weightMod > weightChangeThreshold) and iteration < maxItr:
        traiError, weightMod = nnet.backPropLearning(examplesTrain, alpha)
        iteration += 1
        """
        if iteration%10==0:
            print ('! on iteration %d; training error %f and weight change %f'%(iteration,trainError,weightMod))
    ...
    for input, output in examplesTest:
        ...
        for i in range(len(nnet_output)):
            if round(nnet_output[i]) != output[i]:
                match = False
        if match:
            testCorrect += 1
        else:
    ...
    testAccuracy = float(testCorrect) / len(examplesTest)

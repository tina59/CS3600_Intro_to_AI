...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for key in self.beliefs.keys():
            emissionProb = self.getObservationProb(observation, pacmanPosition, key, jailPosition)
            self.beliefs[key] = self.beliefs[key] * emissionProb
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for p in self.allPositions:
                temp[p] = temp[p] + self.beliefs[oldPos] * newPosDist[p]
        for p in self.allPositions:
            self.beliefs[p] = temp[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        count = int(self.numParticles / numPos)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            tmp[particle] += self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            prev = 1.0
            for i in range(self.numGhosts):
                prev *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))

            tmp[particle] += prev
        newParticles = []
        if tmp.total() != 0:
            for _ in range(self.numParticles):
                if tmp.sample() is not None:
